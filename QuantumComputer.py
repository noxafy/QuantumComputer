...
from utils.quantum.constants import *
from utils.quantum.utils import count_qubits, partial_trace, reorder_qubits
from utils.quantum.state import ket, ket_from_int, dm, unket, plotQ, is_state, ensemble_from_state, is_separable_state
from utils.quantum.hamiltonian import parse_hamiltonian
from utils.quantum.info import *
from utils.quantum.unitary import parse_unitary, get_unitary, Fourier_matrix, get_subunitary, is_separable_unitary
from utils.mathlib import choice, normalize, binstr_from_int, bipartitions
from utils.mathlib.matrix import normalize, is_unitary, is_hermitian, is_diag, trace_product, eigh, outer, is_isometry, kron_eye, random_unitary
from utils.plot import imshow
from utils.utils import is_int, duh, warn, as_list_not_str, nbytes, shape_it
from utils.prob import entropy
...
class QuantumComputer:
    ...
    Example:
        ...
        qc = QC('00+11', track_operators=True)
    ...
    Parameters:
        ...
        as_superoperator (bool): Whether to use superoperator (Choi matrix) representation (True) or Kraus operators (False). Highly recommended to install `scipy` (sparse arrays) for superoperator mode.
        check (int): Check level for input validation (see `../../check levels.md`)
        noise_schedule (None|str|list|function): `None`, a `noise_models` identifier, a set of Kraus operators, or a callback function like this (see `create_benchmark_noise_scheduler` for a factory):
            ```
    ...
    def __call__(self, operators, qubits='all'):
        ...
        if isinstance(operators, str):
            operators = self.parse_unitary(operators)
    ...
    def to_choi(self):
        ...
        if not self.is_matrix_mode():
            ...
            self.to_dm()
        d_out, d_in = self._operators[0].shape
        self._operators = self.choi_matrix().reshape(d_out, d_in, d_out, d_in)
    ...
    def _use_sparse_superoperator(self):
        if self.is_superoperator and "scipy" in sys.modules and self._operators is not None:
